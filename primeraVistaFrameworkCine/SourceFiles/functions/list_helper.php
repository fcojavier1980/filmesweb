...
function listado_flex_6col_1action( $header_title, $total_results, $header, $container, $data, $keys_data, $style_data, $data_list_size=null, $action1=null, $action2=null, $action3=null) { 
		...
		<div class="header_list_sub" style="margin-top: 6px;">
			<div class="header_inside">
				...
				<?php if($value == '&nbsp'){}
						...
						<div id="sort_col_head_<?= $cont ?>" style="display: inline-flex; margin-bottom: 15px; cursor: pointer;"  onclick="sortList_6col_1action(this, 5);">
							<div class="t_col_bold" style="width: <?= $style_data[$cont]?>"><?= $value ?>&nbsp<img src="<?= IMAGES?>icon-abc.png" style="height: 14px; width: 14px;  margin-left: 6px; padding-top: 5px"></div>
						</div>
					<?php } ?>
				<?php 
		...
		foreach ($data as $key => $value) {?>
			<div class="alternate_row_list" id="alternate_row_list_<?= $cont_row ?>">
				<div id="film_id_<?= $value['id'] ?>" style="white-space: nowrap;">	
					<?php
						for($k=0 ; $total_keys > $k ; $k++){ ?>
							<div class="text-data-list sort_col_list_<?= $k ?>" style="display: <?= $k != 6 ? 'inline-flex' : 'none' ?>; width: <?= $style_data[$k]?>;"><?php echo $value[$keys_data[$k]] == $value['created_at'] ? substr($value[$keys_data[$k]] , 0, -8) : $value[$keys_data[$k]] ;?></div>
						<?php } ?>			
						<?php if(isset($action1)){?>
							<div class="sort_col_list_<?= $total_keys_action1 ?>" style="display: inline-block;">
							<form method='post' action='<?= URLWEB . $action1['url'] . $value['id'] ?>' class="action_data_list">
